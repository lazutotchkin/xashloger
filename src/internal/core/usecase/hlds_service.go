...
type HLDSService struct {
	repo        ports.HLDSRepository
	mail        ports.Mailer
	rconFactory ports.RCONFactory
	ttlHours    int
	eventCount  int
	cleanupStep int
}

func NewHLDSService(repo ports.HLDSRepository, mail ports.Mailer, rconFactory ports.RCONFactory) *HLDSService {
	return &HLDSService{
		repo:        repo,
		mail:        mail,
...
func (s *HLDSService) Process(entry *domain.LogEntry) error {
	...
	autokickIp, err := s.repo.IsIPBlackListed(entry.SourceIP)
	if err != nil {
		logrus.Warnf("black list ip check failed: %v", err)
	}

	nameFiltered, err := s.repo.IsFiltered("name", entry.Player)
	if err != nil {
		logrus.Warnf("name filter check failed: %v", err)
	}

	messageFiltered, err := s.repo.IsFiltered("message", entry.Message)
	if err != nil {
		logrus.Warnf("message filter check failed: %v", err)
	}

	if autokick || autokickIp || nameFiltered || messageFiltered {
		logrus.Infof("autokick player %s from %s", entry.Player, entry.ServerIP)

		go func() {
			rconClient := s.rconFactory.New(entry.ServerIP, "secret9")
...
func (s *HLDSService) handleConnect(e *domain.LogEntry) {
	// Проверка IP
	trackedIP, err := s.repo.IsTrackListIp(e.SourceIP)
	if err != nil {
		logrus.Warnf("failed to check IP track list: %v", err)
	} else if trackedIP {
		subject := "Tracked IP connected"
		body := fmt.Sprintf(
			"Tracked IP connected\n\n"+
				"Name: %s\n"+
				"IP: %s\n"+
				"Server: %s\n"+
				"Time: %s\n",
		...
		go func() {
			if err := s.mail.Send(subject, body, "Tracked IP connected"); err != nil {
	...
	// Проверка игрока
	player, err := s.repo.GetUserForTracking(e.Player)
